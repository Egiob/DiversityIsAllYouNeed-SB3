...
class Distribution(ABC):
    ...
    def log_prob_from_params(self, *args, **kwargs) -> Tuple[th.Tensor, th.Tensor]:
        ...
        :return: actions and log probactions_from_params
        """
...
class NormalDistributionONNXable:
    def __init__(self, mean, std):
        self.mean = mean.detach()
        self.std = std.detach()
        self.N = mean.shape[0]

        self.cov = th.eye(self.N)*(self.std**2)
        self.cov_inv = th.eye(self.N)*1/(self.std**2)
        self.pi = th.acos(th.zeros(1))*2


    def log_prob(self, x):
        var = self.std ** 2
        log_scale = th.log(self.std* (2*self.pi)**0.5)
        return -((x-self.mean)**2)/(2*var) - log_scale


    def rsample(self):
        s = th.randn(self.N) * self.std + self.mean 
        return s




class DiagGaussianDistributionONNXable(Distribution):
    """
    Gaussian distribution with diagonal covariance matrix, for continuous actions.
    ...
    def __init__(self, action_dim: int):
        super(DiagGaussianDistributionONNXable, self).__init__()
        self.distribution = None
        self.action_dim = action_dim
        self.mean_actions = None
        self.log_std = None
    ...
    def proba_distribution(self, mean_actions: th.Tensor, log_std: th.Tensor) -> "DiagGaussianDistribution":
        ...
        """
        action_std = th.ones_like(mean_actions) * log_std.exp()
        self.distribution = NormalDistributionONNXable(mean_actions, action_std)
        return self
...
class SquashedDiagGaussianDistributionONNXable(DiagGaussianDistributionONNXable):
    """
    Gaussian distribution with diagonal covariance matrix, followed by a squashing function (tanh) to ensure bounds.
    ...
    def __init__(self, action_dim: int, epsilon: float = 1e-6):
        super(SquashedDiagGaussianDistributionONNXable, self).__init__(action_dim)
        # Avoid NaN (prevents division by zero or log of zero)
        self.epsilon = epsilon
        self.gaussian_actions = None

    def proba_distribution(self, mean_actions: th.Tensor, log_std: th.Tensor) -> "SquashedDiagGaussianDistributionONNXable":
        super(SquashedDiagGaussianDistributionONNXable, self).proba_distribution(mean_actions, log_std)
        return self
    ...
    def log_prob(self, actions: th.Tensor, gaussian_actions: Optional[th.Tensor] = None) -> th.Tensor:
        ...
        # Log likelihood for a Gaussian distribution
        log_prob = super(SquashedDiagGaussianDistributionONNXable, self).log_prob(gaussian_actions)
        # Squash correction (from original SAC implementation)
        # this comes from the fact that tanh is bijective and differentiable
        log_prob -= th.sum(th.log(1 - actions ** 2 + self.epsilon), dim=1)
        return log_prob
